...
class MohaaStatsAPIClient
{
        ...
        if (!empty($handles)) {
            ...
            foreach ($handles as $key => $info) {
                ...
                if ($httpCode === 200 && $response !== false) {
                    ...
                    $isValidSource = ($data !== null || json_last_error() === JSON_ERROR_NONE);

                    if ($info['schema']) {
                        $data = MohaaStatsSchemas::validate($info['schema'], $data);
                    } elseif (is_array($data)) {
                         // Fallback if no schema provided, though we should always use one
                    } else {
                         $data = null;
                    }
    ...
    private function post(string $endpoint, array $data = [], ?string $schema = null): ?array
    {
        ...
        if ($schema) {
            $responseData = MohaaStatsSchemas::validate($schema, $responseData);
        }
        return $responseData;
    }
    ...
    public function deleteTrustedIP(int $forumUserId, int $ipId): ?array { return $this->delete('/auth/trusted-ip/' . $ipId, ['forum_user_id'=>$forumUserId], 'auth_delete_ip'); }
    public function resolvePendingIP(int $forumUserId, int $approvalId, string $action): ?array { return $this->post('/auth/pending-ip/resolve', ['forum_user_id'=>$forumUserId, 'approval_id'=>$approvalId, 'action'=>$action], 'auth_resolve_ip'); }
