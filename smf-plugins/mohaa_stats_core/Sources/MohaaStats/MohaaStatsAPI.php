...
class MohaaStatsAPIClient
{
        ...
        if (!empty($handles)) {
            ...
            foreach ($handles as $key => $info) {
                ...
                if ($httpCode === 200 && $response !== false) {
                    ...
                    if (is_array($data) && !empty($info['schema'])) {
    ...
    public function getLeaderboardCount(string $stat = 'kills', string $period = 'all'): int { $data = $this->get('/stats/leaderboard/global', ['stat'=>$stat,'period'=>$period,'count_only'=>true], 'simple_count'); return (int)($data['total'] ?? 0); }
    ...
    public function getPlayerWeapons(string $guid): ?array { return $this->get('/stats/player/' . urlencode($guid) . '/weapons', [], 'array_of_weapon_stat'); }
    public function getPlayerMatches(string $guid, int $limit = 10, int $offset = 0): ?array { return $this->get('/stats/player/' . urlencode($guid) . '/matches', ['limit'=>$limit,'offset'=>$offset], 'matches_list'); }
    public function getPlayerAchievements(string $guid): ?array { return $this->get('/achievements/player/' . urlencode($guid), [], 'array_of_achievement'); }
    ...
    public function getMatchCount(): int { $data = $this->get('/stats/matches', ['count_only'=>true], 'simple_count'); return (int)($data['total'] ?? 0); }
    public function getMatchDetails(string $matchId): ?array { return $this->get('/stats/match/' . urlencode($matchId), [], 'match'); }
    public function getLiveMatches(): ?array { $orig = $this->cacheDuration; $this->cacheDuration = 10; $data = $this->get('/stats/live/matches', [], 'array_of_match_summary'); $this->cacheDuration = $orig; return $data; }
    public function getMapStats(): ?array { return $this->get('/stats/maps', [], 'array_of_map_stat_summary'); }
    public function getMapDetails(string $mapId): ?array { return $this->get('/stats/map/' . urlencode($mapId), [], 'map_details'); }
    public function getMapHeatmap(string $mapId, string $type = 'kills'): ?array { return $this->get('/stats/map/' . urlencode($mapId) . '/heatmap', ['type'=>$type], 'array_of_heatmap_point'); }
    public function getMapsList(): ?array { return $this->get('/stats/maps/list', [], 'array_of_map_basic'); }
    public function getWeaponsList(): ?array { return $this->get('/stats/weapons/list', [], 'array_of_weapon_basic'); }
    ...
    public function getMatchHeatmap(string $matchId, string $type = 'kills'): ?array { return $this->get('/stats/match/' . urlencode($matchId) . '/heatmap', ['type'=>$type], 'array_of_heatmap_point'); }
    public function initClaim(int $forumUserId): ?array { return $this->post('/auth/claim/init', ['forum_user_id'=>$forumUserId], 'auth_claim_init'); }
    public function initDeviceAuth(int $forumUserId, bool $force = false): ?array { return $this->post('/auth/device', ['forum_user_id'=>$forumUserId, 'force'=>$force], 'auth_device_init'); }
    ...
    public function getMapPopularity(): ?array { return $this->get('/stats/maps/popularity', [], 'array_of_map_popularity'); }
    
    // Stubs
    public function getAchievements(): ?array { return $this->get('/achievements/', [], 'array_of_achievement'); }
    ...
    public function getMatchReport(string $matchId): ?array { return $this->get('/stats/match/' . urlencode($matchId) . '/advanced', [], 'match_advanced'); }
