...
class MohaaStatsAPIClient
{
    ...
    public function getMultiple(array $requests): array
    {
        ...
        foreach ($requests as $key => $request) {
            ...
            $schema = $request['schema'] ?? 'empty'; // Default to empty schema if not provided
        ...
        if (!empty($handles)) {
            ...
            foreach ($handles as $key => $info) {
                ...
                if ($httpCode === 200 && $response !== false) {
                    ...
                    // On failure, we could return default schema structure or null.
                    // Existing behavior was null. User wants NO CRASH.
                    // But Proxy expects null on failure to return 502.
                    // However, for other calls, returning a valid empty structure is safer.
                    // Ideally we return null here to indicate "fetch failed", but if fetch succeeded and parse failed, we return schema default.
                    // MohaaStatsSchemas::validate(null, schema) returns default structure.
                    // Let's stick to null on HTTP error so the caller knows it failed,
                    // BUT strictly validate on success.
                    $results[$key] = null;
                }
    ...
    private function get(string $endpoint, array $params = [], string $schema = 'empty'): ?array
    {
        $url = $this->baseUrl . '/api/v1' . $endpoint;
        if (!empty($params)) $url .= '?' . http_build_query($params);
        $cacheKey = 'mohaa_api_' . md5($url);
    ...
    private function post(string $endpoint, array $data = [], string $schema = 'empty'): ?array
    {
        $url = $this->baseUrl . '/api/v1' . $endpoint;
        $ch = curl_init();
        curl_setopt_array($ch, [
    ...
    private function delete(string $endpoint, array $data = [], string $schema = 'empty'): ?array
    {
        $url = $this->baseUrl . '/api/v1' . $endpoint;
    ...
    public function getPlayerAchievements(string $guid): ?array { return $this->get('/achievements/player/' . urlencode($guid), [], 'player_achievements'); }
    public function getRecentMatches(int $limit = 20, int $offset = 0): ?array { return $this->get('/stats/matches', ['limit'=>$limit,'offset'=>$offset], 'recent_matches'); }
    ...
    public function getMatchHeatmap(string $matchId, string $type = 'kills'): ?array { return $this->get('/stats/match/' . urlencode($matchId) . '/heatmap', ['type'=>$type], 'map_heatmap'); }
    public function initClaim(int $forumUserId): ?array { return $this->post('/auth/claim/init', ['forum_user_id'=>$forumUserId], 'auth_init_claim'); }
    ...
    public function deleteTrustedIP(int $forumUserId, int $ipId): ?array { return $this->delete('/auth/trusted-ip/' . $ipId, ['forum_user_id'=>$forumUserId], 'auth_action_response'); }
    public function resolvePendingIP(int $forumUserId, int $approvalId, string $action): ?array { return $this->post('/auth/pending-ip/resolve', ['forum_user_id'=>$forumUserId, 'approval_id'=>$approvalId, 'action'=>$action], 'auth_action_response'); }
