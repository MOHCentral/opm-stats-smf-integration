...
class MohaaStatsAPIClient
{
    ...
    public function getMultiple(array $requests): array
    {
        ...
        if (!empty($handles)) {
            ...
            foreach ($handles as $key => $info) {
                ...
                // Strict validation and casting (handles nulls/errors by returning default structure)
                if ($schema) {
                    $results[$key] = MohaaStatsSchemas::validate($data, $schema);
                } else {
                    $results[$key] = $data; // Or null if failed
                }

                if ($httpCode === 200 && $results[$key] !== null) {
                    cache_put_data($info['cacheKey'], $results[$key], $this->cacheDuration);
                }
    ...
    private function get(string $endpoint, array $params = [], string $schema = null): ?array
    {
        ...
        // Fix: Only cache if the HTTP request was successful (200 OK)
        // This prevents caching default "zero" data generated during API outages
        if ($httpCode === 200) {
    ...
    private function post(string $endpoint, array $data = [], string $schema = null): ?array
    {
        ...
        if ($schema) {
            $result = MohaaStatsSchemas::validate($result, $schema);
        }
        return $result;
    }
    ...
    public function getLeaderboardCount(string $stat = 'kills', string $period = 'all'): int { $data = $this->get('/stats/leaderboard/global', ['stat'=>$stat,'period'=>$period,'count_only'=>true], 'leaderboard'); return (int)($data['total'] ?? 0); }
    ...
    public function getPlayerWeapons(string $guid): ?array { return $this->get('/stats/player/' . urlencode($guid) . '/weapons', [], 'weapons_list'); }
    ...
    public function getMatchCount(): int { $data = $this->get('/stats/matches', ['count_only'=>true], 'matches_list'); return (int)($data['total'] ?? 0); }
    ...
    public function getMapStats(): ?array { return $this->get('/stats/maps', [], 'maps_list'); }
    public function getMapDetails(string $mapId): ?array { return $this->get('/stats/map/' . urlencode($mapId), [], 'map_details'); }
    public function getMapHeatmap(string $mapId, string $type = 'kills'): ?array { return $this->get('/stats/map/' . urlencode($mapId) . '/heatmap', ['type'=>$type], 'heatmap_data'); }
    ...
    public function getWeaponStats(string $weaponId): ?array { return $this->get('/stats/weapon/' . urlencode($weaponId), [], 'weapon_details'); }
    public function getWeaponLeaderboard(string $weaponId, int $limit = 25): ?array { return $this->get('/stats/weapon/' . urlencode($weaponId) . '/leaderboard', ['limit'=>$limit], 'leaderboard'); }
    public function getMapLeaderboard(string $mapId, int $limit = 25): ?array { return $this->get('/stats/map/' . urlencode($mapId) . '/leaderboard', ['limit'=>$limit], 'leaderboard'); }
    public function getMatchHeatmap(string $matchId, string $type = 'kills'): ?array { return $this->get('/stats/match/' . urlencode($matchId) . '/heatmap', ['type'=>$type], 'heatmap_data'); }

    // Auth
    public function initClaim(int $forumUserId): ?array { return $this->post('/auth/claim/init', ['forum_user_id'=>$forumUserId], 'auth_init'); }
    public function initDeviceAuth(int $forumUserId, bool $force = false): ?array { return $this->post('/auth/device', ['forum_user_id'=>$forumUserId, 'force'=>$force], 'auth_device'); }
    public function getLoginHistory(int $forumUserId): ?array { return $this->get('/auth/history/' . $forumUserId, [], 'auth_history'); }
    public function getTrustedIPs(int $forumUserId): ?array { return $this->get('/auth/trusted-ips/' . $forumUserId, [], 'auth_trusted_ips'); }
    public function getPendingIPApprovals(int $forumUserId): ?array { return $this->get('/auth/pending-ips/' . $forumUserId, [], 'auth_pending_ips'); }
    public function deleteTrustedIP(int $forumUserId, int $ipId): ?array { return $this->delete('/auth/trusted-ip/' . $ipId, ['forum_user_id'=>$forumUserId], 'generic_success'); }
    public function resolvePendingIP(int $forumUserId, int $approvalId, string $action): ?array { return $this->post('/auth/pending-ip/resolve', ['forum_user_id'=>$forumUserId, 'approval_id'=>$approvalId, 'action'=>$action], 'generic_success'); }

    // Server Stats
    public function getGlobalActivity(): ?array { return $this->get('/stats/global/activity', [], 'global_activity'); }
    public function getMapPopularity(): ?array { return $this->get('/stats/maps/popularity', [], 'maps_list'); }
    
    // Stubs / Other
    public function getAchievements(): ?array { return $this->get('/achievements/', [], 'achievements_list'); }
    public function getAchievement(int $id): ?array { return $this->get('/achievements/' . $id, [], 'achievement'); }
    public function getRecentAchievements(): ?array { return $this->get('/achievements/recent', [], 'achievements_list'); }
    ...
    public function getPlayerPlaystyle(string $guid): ?array { return $this->get('/stats/player/' . urlencode($guid) . '/playstyle', [], 'playstyle_data'); }
    public function getMatchReport(string $matchId): ?array { return $this->get('/stats/match/' . urlencode($matchId) . '/advanced', [], 'match_details'); }
...
function MohaaStats_APIProxy(): void
{
    ...
    try {
        ...
        // With strict validation, null might mean API failure, or just no data.
        // If validate() returns default empty structure (e.g. zeros), it is not null.
        // So this check mainly catches connection errors or where no schema was applied and null returned.
        if ($result === null) {
